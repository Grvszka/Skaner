...
const ColorDetailsTable = ({ results }) => {
    ...
    return (
      <div className="flex justify-center py-8">
        ...
        >
          ...
          Rozwiń precyzyjne odczyty stron ({results.length})
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="bg-slate-900 rounded-2xl border border-slate-800 shadow-xl overflow-hidden mt-8 animate-in slide-in-from-bottom-4 duration-500">
      <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-slate-900/50">
        <h3 className="text-xl font-semibold text-slate-100 italic">Lista zidentyfikowanych detekcji barw</h3>
        <button onClick={() => setIsOpen(false)} className="text-slate-400 hover:text-slate-200 text-sm font-medium">Zwiń listę</button>
      </div>
        ...
        <table className="w-full text-left text-sm border-collapse">
          <thead className="bg-slate-950 text-slate-500 uppercase text-[10px] tracking-[0.15em] font-bold">
            <tr>
              ...
              <th className="px-6 py-4 border-b border-slate-800 text-center">Nasycenie barwy</th>
              <th className="px-6 py-4 border-b border-slate-800 text-right">Typ</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-800">
            {currentItems.map((page, idx) => (
              <tr key={idx} className="hover:bg-slate-800/30 transition-colors">
                ...
                <td className="px-6 py-3 text-center text-slate-300 font-mono">
                  {page.saturation.toFixed(2)}%
                </td>
                <td className="px-6 py-3 text-right">
                  {page.isColor ? (
                    <span className="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-purple-900/30 text-purple-400 border border-purple-500/30">
                      Kolorowa
                    </span>
                  ) : (
      ...
      {totalPages > 1 && (
        <div className="p-4 border-t border-slate-800 flex items-center justify-between bg-slate-950/20">
          <p className="text-xs text-slate-500">Wyświetlono {startIndex + 1}–{Math.min(endIndex, results.length)} z {results.length} stron</p>
